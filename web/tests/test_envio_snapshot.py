...
def test_dupla_execucao_com_código_igual_e_resultados_diferentes(request):
    """No caso de reexecução, com código igual e resultado diferentes, deve ser
    feito novo envio de snapshot."""
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=False, args=["--start-maximized"],)
        page = browser.new_page()
        ...
        # Definindo uma nova configuração de testes para forçar outra
        # resultado de execução com o mesmo código
        page.evaluate(
            """
            () => {
